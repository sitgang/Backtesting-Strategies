...
from vtGateway import VtAccountData
...
MAX_REPLACE_COUNTS = 4#最多连续4次


########################################################################
class FirstPointChaseStrategy(CtaTemplate):
    ...
    def __init__(self, ctaEngine, setting):
        ...
        """===========================申明变量=============================="""
        super(FirstPointChaseStrategy, self).__init__(ctaEngine, setting)
        ...
        self.last_pingcang_ID = (EMPTY_STRING,DIRECTION_LONG)#用来追加平仓的
    ...
    def onBar(self, bar):
        ...
        """这个好，撤销之前的单"""
        for orderID in self.orderList2:###追加平仓
        ...
        for orderID in self.orderList:
            ...
            self.cancelOrder(orderID)
            if orderID == self.last_pingcang_ID[0]:###追加平仓
                
                if self.last_pingcang_ID[1] == DIRECTION_LONG:
                    orderID = self.cover(self.closes[-1] + 1, abs(self.pos), stop=True)
                    self.orderList2.append(orderID)
                elif self.last_pingcang_ID[1] == DIRECTION_SHORT:
                    orderID = self.sell(self.closes[-1] - 1, abs(self.pos), stop=True)
                    self.orderList2.append(orderID)
    ...
    def process_ordertrigger(self):
        ...
        if self.di_signal:
            ...
            if self.pos < 0:
                ...
                self.last_pingcang_ID = (orderID,DIRECTION_LONG)
                self.orderList.append(orderID)
                self.volume = 1
                
            if self.volume > MAX_REPLACE_COUNTS:#最多买4次
                
                self.di_signal = False
                return
           
            if self.pos >= 0:#保证了先平仓后开仓
            ...
            if self.pos > 0:
                ...
                self.last_pingcang_ID = (orderID,DIRECTION_SHORT)
                self.orderList.append(orderID)
                self.volume = 1
            
            if self.volume > MAX_REPLACE_COUNTS:#最多做空4次
    ...
    def onTrade(self, trade):
        ...
        price = self.closes[-1]
        if self.volume == MAX_REPLACE_COUNTS+1:
            self.volume = 1
        fenmu = sum(range(self.volume,MAX_REPLACE_COUNTS+1))
        ...
        #平多
            ...
            self.volume = 1
            self.real_volume = int(math.ceil(part*((self.account.available*self.risk_rate)/(price*self.vRate))))
            self.short(price - 1, self.real_volume)
        #开空
        ...
        #平空
            ...
            self.volume = 1
            self.real_volume = int(math.ceil(part*((self.account.available*self.risk_rate)/(price*self.vRate))))
            self.buy(price + 1, self.real_volume)
