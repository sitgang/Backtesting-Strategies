...
########################################################################
class FirstPointMinStrategy(CtaTemplate):
    """关于利用macd的时间力度辅助判断第一买卖点的交易策略"""
    className = 'FirstPointMinStrategy'
    author = u'薛耕'
    ...
    def __init__(self, ctaEngine, setting):
        ...
        """===========================申明变量=============================="""
        super(FirstPointMinStrategy, self).__init__(ctaEngine, setting)
    ...
    def onBar(self, bar):
        ...
        if self.minCount == 2:
            ...
            if not self.minbar:
                ...
                self.minCount += 1
                return
            self.minbar.high = max(bar.high, self.minbar.high)
            self.minbar.low = min(bar.low, self.minbar.low)
            self.minbar.close = bar.close
            self.minCount += 1
    #----------------------------------------------------------------------
